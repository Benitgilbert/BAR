...
export function InventoryManager({
  ...
  async function adjustStock(item: InventoryItem, delta: number) {
    ...
    setError("");
    try {
      const response = await fetch(`/api/items/${item.id}`, { method: "PATCH", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ stockDelta: delta }) });
      const result = (await response.json()) as { item?: InventoryItem; error?: string };
      if (!response.ok || !result.item) throw new Error(result.error || "Unable to adjust stock");
      updateItem(normalizeItem(result.item), `${item.name} stock updated`);
  ...
  return (
    <div className="space-y-6 pb-24 lg:space-y-8 lg:pb-0">
      <section className="flex flex-col justify-between gap-4 sm:flex-row sm:items-end">
        ...
        {canManage && <button type="button" onClick={() => setEditor({ item: null })} className="inline-flex h-12 items-center justify-center gap-2 rounded-xl bg-forest-900 px-4 text-sm font-extrabold text-white shadow-[0_10px_24px_rgba(18,55,42,0.18)]"><Plus className="h-4 w-4" />Register product</button>}
      </section>
